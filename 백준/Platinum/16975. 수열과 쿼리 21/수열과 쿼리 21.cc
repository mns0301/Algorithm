...
void seg_update(vector<long long>& seg, vector<long long>& lazy, int node, int s, int e, int l, int r, int dif) {
...
int main(void) {
	...
	for (int i = 0; i < m; i++) {
		...
		if (t1 == 1) {
			...
			long long t4;
			cin >> t2 >> t3 >> t4;
			seg_update(seg, lazy, 1, 0, n - 1, t2 - 1, t3 - 1, t4);
		}
		else {
			int t2;
			cin >> t2;
			cout << seg_find(seg, lazy, 1, 0, n - 1, t2 - 1, t2 - 1) << "\n";
		}
	}
